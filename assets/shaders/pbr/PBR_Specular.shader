Shader "Unlit/PBR_Specular"
{
    Properties
    {
		// diffuse
    	_AlbedoTex ("Diffuse", 2D) = "white" {}
		_AlbedoTex_ST ("Tiling Offset", Vector) = (1.0, 1.0, 0.0, 0.0)
		_AlbedoColor ("Diffuse Color", Color) = (1.0, 1.0, 1.0, 1.0)
		_AlbedoIntensity ("Diffuse Intensity", range(0, 10)) = 1.0
		...
		_SpecularTex_ST ("Tiling Offset", Vector) = (1.0, 1.0, 0.0, 0.0)
		_SpecularColor ("Specular Color", Color) = (1.0, 1.0, 1.0, 1.0)
		_SpecularIntensity ("Specular Intensity", range(0, 10)) = 1.0
		_GlossniessIntensity ("Glossniess", range(0, 10)) = 1.0
		
		// occlusion
		_OcclusionTex ("Occlusion", 2D) = "white" {}
		_OcclusionTex_ST ("Tiling Offset", Vector) = (1.0, 1.0, 0.0, 0.0)
		_OcclusionIntensity ("Occlusion Intensity", range(0, 10)) = 1.0
    ...
    SubShader
    {
		...
		Pass
		{
			...
			MaterialInfo GetSpecularGlossinessInfo(MaterialInfo info, float3 worldNormal, float4 sgSample)
			{
				info.f0 = _SpecularColor * _SpecularIntensity;
				info.f0 *= sgSample.rgb;

				info.perceptualRoughness = _GlossniessIntensity;
			...
			FragOutput frag (v2f i)
			{
				...
				// specular glossiness
				materialInfo = GetSpecularGlossinessInfo(materialInfo, float3(i.tspace0.z, i.tspace1.z, -i.tspace2.z), sRGBToLinear(tex2D(_SpecularTex, i.uv * _SpecularTex_ST.xy + _SpecularTex_ST.zw)));
				...
				// ao
				float ao  = tex2D(_OcclusionTex, i.uv * _OcclusionTex_ST.xy + _OcclusionTex_ST.zw).r;
